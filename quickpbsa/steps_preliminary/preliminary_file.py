...
def listener(Q, crop_index, jsonfile, parameters, logger):
    while True:
        ...
        # write to log
        logmessage = "trace {:d}, {:d} iterations, runtime {:.2f}s".format(
            trace_index, kv_iter, kv_time
        )
...
def kv_file(
    ...
    # start listener
    pool.apply_async(listener, (Q, crop_index, jsonfile, parameters, logger))
    ...
    for K in trace_indices:
        ...
        job = pool.apply_async(worker, (K, trace, threshold, maxiter, max_memory, Q))
        jobs.append(job)
