...
def posterior_single(data, jpos, means, variances, df, i_in=0, lamb=0.1, gamma0=0.5):
    ...
    if np.all(i >= 0):
        ...
        sic += (
            ...
            + np.log(m - K + 2)
            + np.log(m - K + 1)
            - np.log(m - K + 2 - (m - K + 1) * np.exp(-gamma0 / K))
        )
    else:
        sic = np.float("Inf")
    return sic
            ...
            if len(ind_mult) > 0:
                ...
                stepcomb = list(
                    it.combinations_with_replacement(
                        list(range(1, multstep + 2)), len(ind_mult)
                    )
                )
                ...
                stepcomb = [
                    ...
                    if np.sum(np.array(comb) == multstep + 1) > 0
                ]
                stepcomb = [
                    comb
                    for comb in stepcomb
                    if np.sum(np.array(comb)) > np.sum(np.abs(step_out[ind_mult])) - 3
                ]
                ...
                for comb in stepcomb:
                    ...
                    if multstep > 1:
                        ...
                        sel = np.sum(combarray[:, exclude] == multstep + 1, 1).astype(
                            bool
                        )
                        combarray = combarray[np.invert(sel), :]
                        if np.sum(np.invert(sel)) == 0:
                            break
                    i_complete = np.cumsum(
                        np.hstack(
                            (
                                np.zeros([np.size(combarray, 0), 1], dtype=int),
                                combarray * stepsigns,
                            )
                        ),
                        1,
                    ...
                    for i in np.split(
                        i_complete,
                        np.arange(splitcomb, np.size(i_complete, 0), splitcomb),
                    ):
                        ...
                        if np.size(i, 0) > 0:
                            ...
                            sic = sic_in + np.sum(
                                nphi * np.log(varphi) + diffar / varphi, 1
                            )
    ...
    for stp in (-1, 1):
        for I in range(2, len(steploc) - 1):
            ...
            if I < len(steploc) - 2:
                ...
                diffar[
                    steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I + 2 :
                ] = np.tile(dd, [stepl, 1])
                varphi[
                    steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I + 2 :
                ] = np.tile((i_in[I + 1 :] + stp) * vf + vb, [stepl, 1])
                nphi[
                    steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I + 2 :
                ] = np.tile(np.diff(steploc[I + 1 :]), [stepl, 1])

            # diffs under the diagonal (before added step)
            dar = np.tile(data[steploc[I] : steploc[I + 1]], [stepl, 1])
            dar[np.triu(np.ones([stepl, stepl]), 1).astype(bool)] = (
                i_in[I] * mf + mb
            )  # diff values will be zeros
            ...
            dar = np.tile(data[steploc[I] : steploc[I + 1]], [stepl, 1])
            dar[np.tril(np.ones([stepl, stepl]), 0).astype(bool)] = (
                i_in[I] + stp
            ) * mf + mb
            diffar[steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I + 1] = np.sum(
                (dar - (i_in[I] + stp) * mf - mb) ** 2, 1
            )

            # varphi for the current range
            varphi[steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I] = (
                i_in[I] * vf + vb
            )
            varphi[steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I + 1] = (
                i_in[I] + stp
            ) * vf + vb

            # length for current range
            nphi[steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I] = (
                np.arange(stepl) + 1
            )
            nphi[steploc[I] - jpos[1] : steploc[I + 1] - jpos[1], I + 1] = (
                stepl - np.arange(stepl) - 1
            )
